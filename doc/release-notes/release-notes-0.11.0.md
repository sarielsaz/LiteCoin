Sarielsaz Core version 0.11.0 is now available from:

  <https://sarielsaz.org/bin/sarielsaz-core-0.11.0/>
...
For example, add the following to `sarielsaz.conf`:
...
related to the blockchain in the sarielsaz system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Sarielsaz Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Sarielsaz Core will 
...
For an example script see [sarielsaz-submittx](https://github.com/laanwj/sarielsaz-submittx).
...
- #5334 `cf87536` libsarielsazconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new sarielsaz-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only SarielsazAmountField
- #5753 `0f44672` Add sarielsaz logo to about screen
...
- #5809 `46bfbe7` Add sarielsaz-cli man page
...
- #5849 `41259ca` contrib/init/sarielsazd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Sarielsaz-Qt.app to Sarielsaz-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for sarielsaz-tx)
