...
The RPM spec file provided here is for Sarielsaz-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://sarielsaz.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Sarielsaz git repository](https://github.com/sarielsaz/sarielsaz) but are not part of
...
Currently only `Source10` is used, for the example `sarielsaz.conf` file.
...
in the sarielsaz git repository. At present only an SVG version of the Sarielsaz
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the sarielsaz
...
needed when 0.12.1 is released, a proper fix is already in the Sarielsaz git
master branch.
...
idea to build Sarielsaz Core against a static version of that library compiled
...
The `sarielsaz.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `sarielsaz-server` package. At this point, most RPM based
...
The `sarielsaz-server` package should have SELinux support. How to properly do
...
Sarielsaz-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/sarielsaz/). He has been
packaging Sarielsaz for Fedora at least since 2012.
...
4. I renamed the `sarielsaz` package that contains the Qt GUI to `sarielsaz-core` as
that appears to be how the general population refers to it, in contrast to
`sarielsaz-xt` or `sarielsaz-classic`. I wanted to make sure the general population
