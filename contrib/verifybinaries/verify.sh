...
###   sarielsazcore.org and sarielsaz.org and compares them.
...
WORKINGDIR="/tmp/sarielsaz_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://sarielsazcore.org"
HOST2="https://sarielsaz.org"
BASEDIR="/bin/"
VERSIONPREFIX="sarielsaz-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'sarielsaz-',
...
if [ $? -ne 0 ]; then
   echo "sarielsaz.org failed to provide signature file, but sarielsazcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "sarielsaz.org and sarielsazcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Sarielsaz Core binary release signing key installed?"
   fi
